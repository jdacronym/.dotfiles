...
(defcustom greader-postprocess-chung
  'greader-postprocess-sentence
  "Sets the function used to massage the current text portion. Use this to expand acronyms, substitute phonetic pronunciations of words, and so on"
  :type 'function
  :tag "greader chung postprocessing function")
...
(define-key greader-map (kbd "C-r .") 'greader-read-current-line)
(define-key greader-map (kbd "C-r p") 'greader-read-line-from-point)
(define-key greader-map (kbd "C-r w") 'greader-read-word-at-point)
...
(defun greader-read-current-line ()
  (interactive)
  (greader-read-chung (point-at-bol) (point-at-eol)))

(defun greader-read-line-from-point ()
  (interactive)
  (greader-read-chung (point) (point-at-eol)))

(defun greader-read-word-at-point ()
  (interactive)
  (let ((bounds (bounds-of-thing-at-point 'word)))
    (when (and (not (null bounds))
               (not (null (car bounds)))
               (not (null (cdr bounds)))
               (not (< (cdr bounds) (car bounds))))
      (greader-read-chung (car bounds) (cdr bounds) 'no-line-number))))

(defun greader-read-chung (begin-chung end-chung &optional no-line-number)
  (let ((chung (if (equal begin-chung end-chung)
                   "no characters in chunk. "
                 (buffer-substring begin-chung end-chung)))
        (line (line-number-at-pos)))
    (if chung
        (let ((processed-chung (funcall greader-postprocess-chung chung)))
          (message "postprocessed line: %s" processed-chung)
          (if no-line-number
              (greader-read-asynchronous processed-chung)
            (greader-read-asynchronous (format "Line %s: %s" line processed-chung)))))))

(defcustom greader-postprocess-entries
  '(("macabre" . "mcaaarb")
    ("ICYMI" . "in case you missed it")
    ("pachyderm" . "packy derm")
    ("defvar" . "def-var")
    ("MCAS" . "EmCass")
    ("[IiYy]glesias" . "eeglacieyas")
    ("[Kk]ludge" . "klooj")
    ("[Bb]uffets". "buffeis")
    ("[Hh]ype[Bb]east" . "hypeBeast")
    ("\\b[Cc]oup\\b" . "koo")
    ("\\b[Cc]oups\\b" . "koos")
    ("[Pp]rovenance" . "prahvenance")
    ("NXIUM". "necksium")
    ("[Ee]\\.[Gg]\\." . "EeGee")
    ("vs\\." . "versus")
    ("ABCs" . "EhBeSeez")
    ("moviegoers" . "MovieGoers")
    ("Kantianism" . "CantIanism")
    ("bikelash" . "bikeLash")
    ("[Mm]elatonin" . "melaToenin")
    ("[Aa]pno?ea" . "AppNeeya")
    ("FAQ" . "EfEhCue")
    ("Podesta" . "PuhDesTuh")
    ("Ivanka" . "eVonkuh")
    ...
    ("Fosse\\b" . "Fossey"))
  "A list of regular expressions to replace in chunks of text, and substituations. #'greader-postprocess-sentence uses these to expand acronyms or provide phonetic pronunciations of words that TTS systems have trouble with, like 'macabre' or 'ICYMI'"
  :type 'list
:tag "greader postprocess replacements")

;;(add-to-list 'greader-postprocess-entries '("Fosse\\b" . "Fossey"))
;;(setq greader-postprocess-entries (cdr greader-postprocess-entries))

(defun greader-postprocess-sentence-impl (pairs chung)
  (if (or (null chung) (null pairs))
      chung
    (let ((pair (car pairs))
          (rest (cdr pairs)))
      (greader-postprocess-sentence-impl rest
                                         (replace-regexp-in-string (car pair)
                                                                   (cdr pair)
                                                                   chung
                                                                   'fixedcase)))))

(defun greader-postprocess-sentence (chung)
  (greader-postprocess-sentence-impl greader-postprocess-entries chung))
...
(defun greader-read ()
  ...
  (if (and (greader-timer-flag-p)
           (not (timerp greader-stop-timer)))
      (greader-setup-timers))
  (let ((chung (funcall greader-postprocess-chung
                        (funcall greader-read-chung-of-text))))
    (if chung
        (greader-continue-reading chung)
      (greader-finish-reading))))

(defun greader-continue-reading (chung)
...
(put 'java-mode 'greader-punctuation-chars '(";"))
(put 'magit-revision-mode 'greader-punctuation-chars '("\n"))

(defun greader-punct-p (arg)
  (let ((greader-end-of-sentence
         (cond 
          ((not (null (get major-mode 'greader-punctuation-chars)))
           (get major-mode 'greader-punctuation-chars))
          ((eq major-mode 'java-mode) '(";"))
          ('t greader-end-of-sentence))))
...
(defun greader-get-sentence (&optional direction)
  ...
  (let (sentence)
    (catch 'afterloop
      (save-excursion
        (while (funcall greader-differs (point) (funcall point-limit))
          (setq sentence (concat sentence (greader-get-next-token (following-char))))
          (cond
           ((greader-end-sentence-p)
...
(defun greader-get-next-token (next-char)
  "get the token to represent the passed in character
returns string"
  (let ((token (string next-char))
        (next-token-fn (get major-mode 'greader-fnext-token-fn)))
    (cond
     ((not (null next-token-fn))
      (funcall next-token-fn  token))
     ;; ((eq major-mode 'java-mode)
     ;;  (greader-java-token token))
     ('t token))))

(defvar-local last-token nil
  "string representation of the last character token processed")
(defvar-local search-success nil
  "indicate whether we are in the arg of a if/for/while/try block")
 
(defun greader-java-token (token)
  (let ((next-token
         (cond ((string-equal token "/")
                (cond ((string-equal last-token "/") " single-line comment: ")
                      ((string-equal last-token "*") " end multi-line comment. ")
                      ((eq (char-after (+ 1 (point))) ?/) "")
                      ((eq (following-char) ?*) "")
                      ('t " slash ")))
               ((string-equal token "\\") " backslash ")
               ((string-equal token "*")
                (cond ((eq (preceding-char) ?/) " multi-line comment begins: ")
                      ((eq (char-before (- (point) 1)) ?/) "")
                      ('t " asterisk ")))
               ((string-equal token "-") " dash ")
               ((string-equal token "\"")
                (cond ((eq (char-after (+ (point) 1)) ?\") "")
                      ((eq (char-after (- (point) 1)) ?\") " empty string ")
                      ('t " quote ")))
               ((string-equal token ".") " dot ")
               ((string-equal token "{") " start block. ")
               ((string-equal token "}") ". end block. ")
               ((string-equal token "(") 
                (save-excursion
                  (goto-char (+ (point) 1))
                  (if (string-equal (string (following-char)) ")")
                      ""
                    (cond ((> (point) (greader-search-backward "if")) "")
                          ((> (point) (greader-search-backward "for")) "")
                          ((> (point) (greader-search-backward "while")) "")
                          ((> (point) (greader-search-backward "try")) "")
                          ('t (setq search-success nil) " with arguments: ")))))
               ((string-equal token ")")
                (setq search-success nil)
                (if (string-equal last-token "(")
                    (if (null search-success) ". No arguments. "
                      "")
                  (if (null search-success) ". End arguments. "
                    "")))
               ...
               ((string-equal token "=")
                (cond ((eq (char-after (+ (point) 1)) ?=) "")
                      ((eq (char-after (- (point) 1)) ?=) " equals ")
                      ((eq (char-after (- (point) 1)) ?!) " not equals ")
                      ('t " equal ")))
               ((string-equal token "@") " annotation, ")
               ((string-equal token "*") " star ")
               ((string-equal token "_") " underscore ")
               ((string-equal token ":") " colon ")
               ((string-equal token ">")
                (if (eq (char-after (- (point) 1)) ?<)
                    " empty type identifier "
                  ", greater than "))
               ((string-equal token "<")
                (if (eq (char-after (+ (point) 1)) ?>)
                    ""
...
(put 'java-mode 'greader-next-token-fn #'greader-java-token)

(defun greader-search-backward (string)
  (let ((result (search-backward string (- (point) 10) 't)))
    (if (null result)
        (point)
      (progn (setq search-success 't)
             result))))
...
(put 'magit-revision-mode 'greader-line-oriented 't)

(defun greader-end-sentence-p ()
  (if (get major-mode 'greader-line-oriented)
      (eolp)
    (catch 'endsentence
      (save-excursion
        (if (eobp)
